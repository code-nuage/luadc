...
function luadc.input(listener, input)
    ...
    if input == listener then -- Listen to an input to put a condition in
        return true
    end
end
...
function luadc.execute_menu(menu)
    menus[menu].functions[menus[menu].selector]() -- Execute the current selected function in the menu
end
...
function luadc.draw_text(string, x, y)
    screen:mvaddstr(y, x, string)
end
